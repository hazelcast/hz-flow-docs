...
Once this has had a minute or two to start the application and set everything up, you can navigate to:
http://localhost:9021 and start using {short-product-name}.

=== Live reload
The devleoper environment of {short-product-name} is configured with live reload, so any changes you make in your taxi project are automatically picked up.
