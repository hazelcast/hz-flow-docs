...
import oktaSamlSingleSignOnUrl from '@/img/okta-saml-sign-on-url.png'
import oktaSamlAudienceUrl from '@/img/okta-saml-audience-url.png'
...
<Callout type={'note'} title="Don't forget!">
  You must also configure AWS Cognito link:/deploying/authorization#aws-cognito[role-based access control] +
</Callout>

=== Azure+++<Callout type="warning" title="Azure AD B2C Not Supported">+++Unfortunately, at this time Azure AD B2C is not supported, as it does not provide support for <a href={'https://learn.microsoft.com/en-us/answers/questions/818404/role-based-access-for-webapi-in-azure-b2c'}>exposing roles in the published JWT</a>. However, Azure Active Directory (Entra) is supported, as described below.+++</Callout>+++
...
<Callout type={'note'} title="Don't forget!">
  You must also configure Azure link:/docs/deploying/authorization#azure[role-based access control]
</Callout>
