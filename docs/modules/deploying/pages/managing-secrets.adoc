...
For example:+++<SnippetGroup>+++```hocon connections.conf jdbc { my-connection { // \... other params omitted for brevity \... connectionParams { password = $\{postgres_password} // Reads the environment variable "postgres_password" } } } ``` ```hocon env.conf postgres_password=hello123 ```+++</SnippetGroup>+++
