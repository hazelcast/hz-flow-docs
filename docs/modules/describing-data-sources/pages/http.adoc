...
Orbital has first-class support for working with xref:open-api.adoc[OpenAPI specs] and xref:soap.adoc[SOAP WSDLs]. The following topic is intended for consuming HTTP services without a separate spec.
