...
When executed as the Admin user, all fields are returned.  When executed as the `user` user, the `price` field is excluded.   To execute as the `user` user, log out and log back in as the `user` user.
...
When executed as the Admin user, all fields are returned including the `Ticker` field.  When executed as the `user` user, the `Ticker` field is obfuscated.   To execute as the `user` user, log out and log back in as the `user` user.
